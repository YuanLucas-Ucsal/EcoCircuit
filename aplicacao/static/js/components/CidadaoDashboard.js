function CidadaoDashboard({ 
  ...
  userId 
    ...
    React.useEffect(() => { 
        if (screen === 'mapa') { 
        fetch('http://localhost:8000/api/localizacoes') 
            .then(res => res.json()) 
            .then(dados => { 
    ...
    React.useEffect(() => { 
        if (screen === 'carteira') { 
        fetch(`http://localhost:8000/api/cidadaos/${userId || 1}/vouchers`) 
            .then(res => res.json()) 
            .then(dados => { 
    ...
    const lidarComDescarte = async () => {
        ...
        try {
        const response = await fetch('http://localhost:8000/api/cidadaos/descarte-sucesso', {
            method: 'POST',
