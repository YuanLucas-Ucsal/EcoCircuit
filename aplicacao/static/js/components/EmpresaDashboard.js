function EmpresaDashboard({ screen, setScreen, sidebarClass, cardClass, inputClass, globalLimits, showToast }) {
    ...
    // ADICIONADO: Injeta o ID da empresa corporativa atual logada (Exemplo: 1 para testes)
    const empresaIdLogada = 1; 
    ...
    fetch('http://localhost:8000/api/ofertas/criar', {
        ...
        body: JSON.stringify(corpoRequisicao) // Envia o objeto modificado
    })
    .then(resposta => {
        if (resposta.ok) {
    ...
    const [listaVouchers, setListaVouchers] = React.useState([]);
    const empresaIdLogada = 1; // ID fixo demonstrativo para a empresa ativa

    // Busca o histórico de vouchers consumidos do banco sempre que a tela mudar para 'history'
    React.useEffect(() => {
    if (screen === 'history') {
        fetch(`http://localhost:8000/api/empresas/${empresaIdLogada}/vouchers`)
        .then(res => res.json())
        .then(dados => {
            if (Array.isArray(dados)) {
            setListaVouchers(dados);
            }
        })
        .catch(err => console.error("Erro ao carregar histórico de cupons:", err));
    }
    }, [screen]);
            ...
            <main className="flex-1 p-8 space-y-5">
                ...
                {screen === 'history' && (
                    <div className={`p-5 rounded-3xl border overflow-hidden ${cardClass}`}>
                        ...
                        <tbody>
                            {/* MAPEAMENTO DINÂMICO DOS VOUCHERS DO BANCO */}
                            {listaVouchers.map((voucher) => (
                            <tr key={voucher.id} className="border-b border-zinc-700/10">
                                <td className="py-3 font-mono font-bold uppercase">{voucher.codigo}</td>
                                <td className="py-3">{voucher.oferta_titulo}</td>
                                <td className="py-3">{voucher.utilizacao}</td>
                                <td className="py-3 text-emerald-500 font-bold">{voucher.status}</td>
                            </tr>
                            ))}
                            {listaVouchers.length === 0 && (
                            <tr>
                                <td colSpan="4" className="py-4 text-center text-gray-500 italic">
                                Nenhum cupom consumido por clientes até o momento.
                                </td>
                            </tr>
                            )}
                        </tbody>
