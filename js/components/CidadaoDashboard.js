function CidadaoDashboard({ 
    ...
    React.useEffect(() => {
        if (screen === 'mapa') {
            ...
            const map = L.map('mapa-real-core', {
                zoomControl: false
            }).setView([-12.9796, -38.4539], 14);
            ...
            const marker1 = L.marker([-12.9796, -38.4539]).addTo(map);
            marker1.bindPopup("<b>Contêiner Shopping Salvador</b><br>Capacidade: 72%").openPopup();
            marker1.on('click', () => {
                setSelectedContainer({ 
                    name: 'Shopping Salvador', 
                    location: 'Av. Tancredo Neves, Piso L1', 
                    volume: 72, 
                    blocked: false 
                });
            });

            const marker2 = L.marker([-12.9840, -38.4601]).addTo(map);
            marker2.bindPopup("<b class='text-red-500'>Contêiner Pituba</b><br>🚨 LOTADO (100%)");
            marker2.on('click', () => {
                setSelectedContainer({ 
                    name: 'Pituba Supermercado', 
                    location: 'Av. Manoel Dias da Silva', 
                    volume: 100, 
                    blocked: true 
                });
            });
