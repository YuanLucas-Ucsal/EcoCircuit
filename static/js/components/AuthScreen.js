function AuthScreen({ theme, userRole, setUserRole, setIsAuthenticated, cardClass, inputClass, showToast }) {
    ...
    const handleSubmit = (e) => {
        ...
        const BASE_URL = 'https://ecocircuit-2oas.onrender.com';

        let url = `${BASE_URL}/api/auth/login`;
        if (authMode === 'register') url = `${BASE_URL}/api/auth/cadastro`;
        if (authMode === 'forgot') url = `${BASE_URL}/api/auth/recuperar-senha`;
