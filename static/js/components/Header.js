function Header({ theme, setTheme, isAuthenticated, setIsAuthenticated }) { 
  
  const handleLogout = () => {
    fetch('https://ecocircuit-2oas.onrender.com/api/auth/logout', { method: 'POST' })
      .then(() => {
      ...
      <div className="flex items-center gap-3"> 
        ...
        {isAuthenticated && (
          <button 
            ...
            className="h-9 px-4 rounded-xl flex items-center justify-center bg-red-500/10 hover:bg-red-500 text-red-400 hover:text-white border border-red-500/20 text-xs font-bold cursor-pointer transition-all active:scale-90"
          >
